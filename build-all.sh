...
while read -r PKG PKG_DIR; do
	...
	if [ -n "${TERMUX_DEBUG_BUILD}" ]; then
		echo "\"$BUILDSCRIPT\" -a \"$TERMUX_ARCH\" $TERMUX_DEBUG_BUILD --format \"$TERMUX_FORMAT\" --library $(package__is_package_name_have_glibc_prefix "$PKG" && echo "glibc" || echo "bionic") ${TERMUX_OUTPUT_DIR+-o $TERMUX_OUTPUT_DIR} $TERMUX_INSTALL_DEPS \"$PKG_DIR\""
	fi
	echo -n "Building $PKG... "
	BUILD_START=$(date "+%s")
	"$BUILDSCRIPT" -a "$TERMUX_ARCH" $TERMUX_DEBUG_BUILD --format "$TERMUX_FORMAT" \
		--library $(package__is_package_name_have_glibc_prefix "$PKG" && echo "glibc" || echo "bionic") \
